...
`,
  ...
  mounted: function () {
    ...
    client.setup(settings.port, settings.currentUser+'-browser', settings.server, (username, party) => {
        ...
        let idList = party.map(mon => mon.pokemon.species).join('|')

        fetch('https://api.pokemontcg.io/v1/cards?setCode=base1|base2|basep|ex3&supertype=pokemon&nationalPokedexNumber=' + idList)
          ...
          .then(cards => {
            vm.players[username] = party.map(function (pokemonWrapper) {
                ...
                try {
                  transformed.img = cards.cards.find(card => card.nationalPokedexNumber === transformed.species).imageUrl
                } catch (e) {
                  console.log(e, transformed)
                }

                console.log(transformed.img)
                return transformed
